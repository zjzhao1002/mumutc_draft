...
Table.~\ref{table:LVLLed} lists the runnings of three sample parameter points: 
1) $c_{9}=-1.0, c_{10}=-0.1$, as an example of general new physics scenario, 
2) $c_{9}=c_{10}=0.25$, as an example that $C_{qe}$ is the main contribution on at scale beyond $M_Z$,
2) $c_{9}=-c_{10}=-0.39$, as an example that $C^{(1)}_{lq}$ and $C^{(3)}_{lq}$ are the main contributions at scale beyond $M_Z$.
We can safely say that the operators mixing in RGE running has not strong effects to measure the SMEFT at high energy machine.
...
\begin{table}
  ...
  \hline
  &      $c_9=-1.00$        &     $c_9=0.25$         &       $c_9=-0.39$         \\
  &      $c_{10}=-0.10$     &     $c_9=c_{10}$       &       $c_9=-c_{10}$       \\
  \hline  
...
For the $\mu^+\mu^-\to tc$ and $bs$ processes, the input from low energy scale is $c_{9}=-c_{10}=-0.39$.  
The cross section of the main signal process $\mu^+ \mu^- \to tc$ derived from the 4-fermion interactions are proportional to collision $s^2$, 
...
In Fig.~\ref{ecm}, the best fit $c_{9}=-c_{10}=-0.39$ in Ref.~\cite{Altmannshofer:2021qrr} have been considered. 
...
in the case of $c_{9}=-c_{10}=-0.39$.
We have considered two simple scenarios at EW scale ($M_Z$): 1) $C^{(1)}_{lq}=L^{V,LL}_{ed} \frac{\Lambda^2}{v^2}$ and $C^{(3)}_{lq} = 0$; 2) $C^{(1)}_{lq}=3 C^{(3)}_{lq}=\frac{3}{4} L^{V,LL}_{ed} \frac{\Lambda^2}{v^2}$.
...
we input events generated with $c_{9}=-c_{10}=-0.39$ to PYTHIA8~\cite{Bierlich:2022pfr} for parton shower and hadronization, 
...
In such a high energy collider, parton shower can radiate some high energy particles and can be detected as a hard jet. 
To reduce these radiations, we implement a cut $E(j)>500$ GeV to jets for each event. 
...
%  \caption{The number of events before and after each cut are listed, where $\sigma$ is defined as $\sigma=S/\sqrt{S+B}$. We assume the luminosity is $30$ ab$^{-1}$ at 10 TeV muon collider. The details of these cuts are described in the text.\label{table:tc:cut}}
  \caption{The number of events before and after each cut are listed, where $\sigma$ is defined as $\sigma=S/\sqrt{S+B}$. We assume the luminosity is $10$ ab$^{-1}$ at 10 TeV muon collider. The details of these cuts are described in the text.\label{table:tc:cut}}
\end{table}
\end{center}
...
%  \caption{The number of events before and after each cut is listed. We assume the luminosity is $30$ ab$^{-1}$ at 10 TeV muon collider. The details of these cuts are described in the text.\label{table:bs:cut}}
  \caption{The number of events before and after each cut is listed. We assume the luminosity is $10$ ab$^{-1}$ at 10 TeV muon collider. The details of these cuts are described in the text.\label{table:bs:cut}}
\end{table}
\end{center}

Fig.~\ref{fig:cll1d} shows the $2\sigma$ and $3\sigma$ bounds on $C^{X}_{LL}$. 
Based on our analysis, $C^{tc}_{LL}$ can be constrained to $[-0.0177, +0.0177]$ ($[-0.0220, +0.0220]$) at $2\sigma$ ($3\sigma$) limit at 10 TeV muon collider with luminosity $\mathcal{L}=10$ ab$^{-1}$, 
while $C^{bs}_{LL}$ can be constrained to $[-0.0226, +0.0226]$ ($[-0.0279, +0.0279]$) at $2\sigma$ ($3\sigma$) limit. 
...
<<<<<<< HEAD
Similarly, we implement same cuts to analyse the case that only $C_{qe}$ is switched on. 
Fig.~\ref{fig:cqe1d} shows the $2\sigma$ and $3\sigma$ bounds on $C_{qe}$. 
There is no wonder that these bounds are close to that we obtain for $C^{X}_{LL}$.
...
\begin{figure}[htbp]
  ...
  \thesubfigure
  \label{fig:clltc1d}
  \includegraphics[width=0.4\textwidth]{cqetc.pdf}}
  \subfigure{
  \thesubfigure
  \label{fig:cllbs1d}
  \includegraphics[width=0.4\textwidth]{cqebs.pdf}}
  \caption{The $2\sigma$ and $3\sigma$ bounds of $C_{qe}$ obtained by measuring (a) $tc$ and (b) $bs$ final state at 10 TeV muon collider with luminosity $\mathcal{L}=10$ ab$^{-1}$.}\label{fig:cqe1d}
=======
\begin{figure}
...
>>>>>>> 842de72398e41c518127f855f83bc4ed5d39ad7e
\end{figure}
